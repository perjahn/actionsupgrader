...
class Git
{
    ...
    {
        ProcessStartInfo startInfo = new("git", $"config {configName} \"{value}\"") { WorkingDirectory = repoFolder };
        Process.Start(startInfo)?.WaitForExit();
    }

    public static string GetCurrentBranch(string repoFolder)
    {
        ProcessStartInfo startInfo = new("git", $"rev-parse --abbrev-ref HEAD") { WorkingDirectory = repoFolder, RedirectStandardOutput = true };
    ...
    public static void Commit(string repoFolder, string message)
    {
        ...
        ProcessStartInfo startInfo = new("git", $"commit -a -m \"{message}\"") { WorkingDirectory = repoFolder };
        Process.Start(startInfo)?.WaitForExit();
    }

    public static void Push(string repoFolder, string remoteBranch, bool dryRun)
    {
        ProcessStartInfo startInfo = new("git", $"push origin HEAD:{remoteBranch}") { WorkingDirectory = repoFolder };
